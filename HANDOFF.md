...
- 現職：**CMoney 產品經理（正職，2026/09 起）**。2025/06–2026/08 為產品企劃實習（APM），經歷拆成兩筆呈現（先 PM 後 APM）；
  現負責 VIP 作者事業「林恩如」作者產品線。**對外只寫作者名（權證小哥、林恩如），不寫個別產品名。全站不再出現「實習」「APM」當現職。**
...
1. **TapAble · BUILDMODE GEN-AI HACKATHON 2026**（🥈 第二名 · 160 隊，代表作）
   ...
   - 隊名 ChatKTV（T161），5 人（北科大 × 銘傳）。**分工（使用者 2026-09-08 定稿）**：本人隊長＋統籌＋**約 95% 程式**（AI agent 協作）＋產品定義＋田野驗證方法；
     一位組員約 5% 程式＋簡報＋上台；另三位做研究素材、影片腳本與拍攝，其中一位在賽前完成三位視障者訪談。
...
8. **第 44 屆測量及空間資訊研討會 3S 創客競賽**（🏅 入圍獎，`noPage` 但有 `demoPath`）— 瑤光深鯨空間資訊版，決賽 2026/08/21 政大。
   無獨立頁，卡片內有「開啟互動 Demo」連到 `public/yao-3s-demo/index.html`（WebGIS，Leaflet，iPad 可用；內容為功能層次，2026-09-08 掃過無敏感字）。
...
- 工作經歷順序（新→舊）：CMoney PM（2026/09–）→ CMoney APM（2025/06–2026/08）→ 香港喜來登酒店 → 香港東華三院 → Compass Group。

## 5. 設計決策（改動前先知道）

- 首頁能力區叫「**四種能力，一個完整的產品人**」，刻意不用「四個作品」（作品數量會變）。
- **首頁作品區兩層（2026-09-08 重構）**：上層「三個代表作」= 3 張大卡（`featured: true`，有 `coverImg`／`hook`／`stat`；TapAble 另有 `timeline` 四站與 `startup: true` 的「🚀 創業進行中」標）；
  下層「其他競賽與專案」= 一行一列的清單（`series` 欄位標「TapAble 系列」，讓同一題目的多場競賽讀起來是進化不是灌水）。不要再回到九張等大卡片。
- 代表作三個：TapAble、AWS、法巴。使用者明說「這是我的頭三」。
- 封面圖：TapAble `/tapable/deck-solution.jpg`、AWS `/ppa/cover.jpg`（demo 截圖）、法巴 `/bnp/business-model.png`（原封面 `/bnp/cover.png` 有組員 email，不當縮圖）。
- Hero 下三個數字由 `projects[]` 直接數出來（場次、第二名數），第三個「1 個開源產品」寫死。
- 創業只標「創業進行中」四個字，不多寫（使用者在職）。
